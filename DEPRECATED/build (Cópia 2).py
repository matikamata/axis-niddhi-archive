...
def _copy_audio_files(csl_root: Path) -> None:
    """
    Copia MP3s de CSL/meta/pronunciation/ para output/audio/ incrementalmente.
    Usa mtime para skip — sem hash completo (arquivos MP3 são grandes e estáveis).
    Nunca aborta se diretório ausente.
    """
    ...
    logger.info(f"🎵 Audio: {copied} copiados | {skipped} sem alteração → {audio_dst.name}/")


def _generate_pronunciation_manifest(csl_root: Path, glossary: Dict[str, Any]) -> None:
    """
    Gera pronunciation_manifest.json: { termo → "audio/termo.mp3" }
    Inclui apenas termos com MP3 real no output/audio/.
    Nunca aborta se glossário ou diretório ausente.
    """
    audio_src = csl_root / "meta" / "pronunciation"
    manifest: Dict[str, str] = {}

    if not audio_src.exists() or not glossary:
    ...
    for term in glossary:
        # Tentar match direto: dhamma.mp3
        candidate = audio_src / f"{term}.mp3"
        if candidate.exists():
            manifest[term] = f"audio/{term}.mp3"
            continue
        # Fallback NFKD: diacríticos → ASCII slug
        slug = unicodedata.normalize("NFKD", term).encode("ascii", "ignore").decode("ascii")
        slug = re.sub(r"[^a-z0-9]+", "-", slug.lower()).strip("-")
        candidate_slug = audio_src / f"{slug}.mp3"
        if candidate_slug.exists():
            manifest[term] = f"audio/{candidate_slug.name}"
...
def main() -> None:
    ...
    # Persistir template_hash no cache para próxima run
    if CACHE_FILE.exists():
        try:
            cache_data = json.loads(CACHE_FILE.read_text())
    ...
    # ── 8. SERVICE WORKER (hardened — ÚLTIMO) ─────────────────────────────
    logger.info("▶ Fase 8/8: Service Worker...")
    _inject_build_id_into_sw()
